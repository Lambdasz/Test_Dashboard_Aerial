...
export const Route = createFileRoute("/trees")({ component: TreesPage });
...
function TreesPage() {
  ...
  const visible = TREE_DETECTIONS.filter(
    (t) => t.confidence >= range[0] && t.confidence <= range[1],
  );

  const hist = useMemo(() => {
    const bins = Array.from({ length: 10 }, (_, i) => ({ bin: (i / 10).toFixed(1), count: 0 }));
    TREE_DETECTIONS.forEach((t) => {
      bins[Math.min(9, Math.floor(t.confidence * 10))].count += 1;
    });
    return bins;
  }, []);
  ...
  return (
    <>
      ...
      <div className="aap-grid">
        ...
        <div className="aap-span-6">
          <ChartCard title="Confidence histogram">
            <ResponsiveContainer width="100%" height={250}>
              <BarChart data={hist}>
                ...
                <YAxis fontSize={11} />
                <Tooltip />
                <Bar dataKey="count" fill="#2d72d2" radius={[3, 3, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </ChartCard>
        </div>
        ...
        <div className="aap-span-6">
          <ChartCard title="Trees per plot">
            <ResponsiveContainer width="100%" height={260}>
              <BarChart data={perPlot}>
                <CartesianGrid strokeOpacity={0.15} vertical={false} />
                <XAxis dataKey="name" fontSize={11} />
                <YAxis fontSize={11} />
                <Tooltip />
                <Bar dataKey="trees" fill="#1c6e42" radius={[3, 3, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </ChartCard>
        </div>
